...
public class SaltSeparatorContainer extends ContainerBase {
	...
	public SaltSeparatorContainer(InventoryPlayer invPlayer, IItemHandler separator) {
		super(invPlayer, separator);

		// Battery
		this.addSlotToContainer(new SlotNonRetarded(separator, 0, 152, 81));
		// Schematic
		this.addSlotToContainer(new SlotNonRetarded(separator, 1, 35, 126));
		// Upgrades
		this.addSlots(separator, 2, 152, 108, 2, 1);
		/*
		// Solid Input
		this.addSlots(separator, 4, 8, 99, 1, 3);
		// Solid Output
		this.addOutputSlots(invPlayer.player, separator, 7, 80, 99, 1, 3);
		// Fluid Input
		this.addSlots(			separator, 10, 8, 54, 1, 3);
		this.addTakeOnlySlots(	separator, 13, 8, 72, 1, 3);
		// Fluid Output
		this.addSlots(			separator, 16, 80, 54, 1, 3);
		this.addTakeOnlySlots(	separator, 19, 80, 72, 1, 3);*/

		// Fluid Input
		this.addSlots(			separator, 4, 26, 54, 1, 2);
		this.addTakeOnlySlots(	separator, 6, 26, 72, 1, 2);
		// Fluid Output
		this.addSlots(			separator, 8, 80, 54, 1, 3);
		this.addTakeOnlySlots(	separator, 11, 80, 72, 1, 3);
		// Solid Output
		//this.addOutputSlots(invPlayer.player, separator, 12, 116, 18, 4, 0);
		...
		if(slot != null && slot.getHasStack()) {
			...
			if(index < tileSize) {
				...
				if(Library.isBattery(slotOriginal)) {
					...
					if (isFluidFull) {
						if(!this.mergeItemStack(slotStack, 4, 6, false)) return ItemStack.EMPTY;
						handled = true;
					} else if (isFluidEmpty) {
						if(!this.mergeItemStack(slotStack, 8, 11, false)) return ItemStack.EMPTY;
						handled = true;
					}

					// Fallback to solid input if not a fluid container
					if(!handled) {
						//if(!InventoryUtil.mergeItemStack(this.inventorySlots, slotStack, 12, 16, false)) return ItemStack.EMPTY;
						return ItemStack.EMPTY;
					}
				}
			}
